	...
	******************************/
	...
	var loadNextImage = function(active) {
		var next =  active.next(".img-wrapper").length ? active.next(".img-wrapper") : GALLERY.find('.img-wrapper:first'),
			nextThumb = $('#container').find('[data-thumb-id="' + next.data('img-id') + '"]');

		// Setting next image & thumb properties
		$('.slideShow').find('.thumb').removeClass('active');
	...
	var loadPrevImage = function(active) {
		var prev =  active.prev(".img-wrapper").length ? active.prev(".img-wrapper") : GALLERY.find('.img-wrapper:last'),
			prevThumb = $('#container').find('[data-thumb-id="' + prev.data('img-id') + '"]');
		// Setting next image & thumb properties
	...
	loadClickedImage = function(id) {
		//var image =  $('#container').find('[data-img-id="' + id + '"]'),
			var	imgThumb = $('#container').find('[data-thumb-id="' + id + '"]'),
			//currActive = $('#container').find('.img-wrapper.active');
	...
	var scrollThumbnails = function (thumb) {
		...
		if(x < 0) {
			first = $('#container').find('.thumb:first');
